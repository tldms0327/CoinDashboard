...
class IndicatorCalculator:
    def __init__(self, batch, epoch_id):
        self.batch = batch
        self.epoch_id = epoch_id

    def MvaLong(self):
        mva_long = self.batch.groupby('symbol') \
            .agg(avg('close_price').alias('mva_long')) \
            .withColumn("index", lit("mva_long"))
        mva_long.write.mode('append').json("s3://coinboard/indicator/mva_long")
        print("=== mva_long DONE ===")

    def MvaShort_Cci(self):
        mva_short = self.batch.filter(self.batch.opentime > datetime.now() - timedelta(hours=1)). \
            groupby('symbol').agg(avg('close_price').alias('mva_short')).withColumn("index", lit("mva_short"))

        temp = self.batch.sort(asc('opentime')).groupby('symbol'). \
            agg(min('close_price').alias('min'), max('close_price').alias('max'), last('close_price').alias('last')). \
            withColumnRenamed("symbol", "symbol_b")
        join_exp = [mva_short.symbol == temp.symbol_b]
        cci = temp.withColumn('sum', (col("max") + col("min") + col("last")) / 3).join(mva_short, join_exp, 'inner'). \
    ...
    def Bollinger(self):
        bollinger_band = self.batch.groupBy("symbol").agg(avg("close_price").alias("avg"),
    ...
    def Stochastic(self):
        df1 = self.batch.groupBy("symbol").agg(max("opentime").alias("latest_time"),
                                               max("close_price").alias("max_price"),
                                               min("close_price").alias("min_price")) \
            .withColumn("diff", col("max_price") - col("min_price"))
        df2 = self.batch.withColumnRenamed("opentime", "opentime_B") \
            .withColumnRenamed("symbol", "symbol_B") \
    ...
    def RSI(self):
        ...
        partitioned_batch = self.batch.select("opentime", "symbol", "close_price",
                                              row_number().over(
                                                  Window.partitionBy("symbol").orderBy(col("opentime").desc())).alias(
                                                  "row_num"))
        # lag
        Windowspec = Window.orderBy("symbol")
        df_prev = partitioned_batch.withColumn('prev_price', lag(self.batch['close_price'], 1).over(Windowspec))
        # calculate 1m rate
